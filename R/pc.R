...
#' @param estimation_method A character string, either "u" or "v". If "u", U-statistics are used, if "v", V-statistics are used.
...
#' pcov(X, Y, estimation_method = "u", n_threads = 4)
#'
#' @export
pcov <- function(X, Y, estimation_method = "u", n_threads = 4) {
    pcov_cpp(X, Y, estimation_method, n_threads)
}
...
#' X <- matrix(rnorm(100 * 34, 1), 100, 34)
#' Y <- matrix(rnorm(100 * 62, 1), 100, 62)
#' pcor(X, Y, estimation_method = "u", n_threads = 4)
#'
#' @export
pcor <- function(X, Y, estimation_method = "u", n_threads = 4) {
    pcor_cpp(X, Y, estimation_method, n_threads)
}
...
#' pcor.test(X, Y, estimation_method = "v", times = 199, n_threads = 4)
#'
#' @export
pcor.test <- function(X, Y, estimation_method = "u", times = 199, n_threads = 4) {
    pcor_test_cpp(X, Y, estimation_method, times, n_threads)
}
